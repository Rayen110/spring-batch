...
public class BatchConfiguration {
    ...
    //4、配置写
    @Bean
    public ItemWriter<WriterSO> userWriteItem() {
        FlatFileItemWriter<WriterSO> fileWrite = new FlatFileItemWriter<>();
    ...
    /*按指定格式写入文件*/
    private class UserLineAggregator implements LineAggregator<WriterSO> {
        @Override
        public String aggregate(WriterSO item) {
            return item.getId() + ", " + item.getFullName()+ ", " + item.getRandomNum();
        }
    ...
    public Step step1(StepBuilderFactory stepBuilderFactory, ItemReader<RecordSO> reader,
                      ItemWriter<WriterSO> writer, ItemProcessor<RecordSO, WriterSO> processor) throws Exception {
